...
IMAGE_HEIGHT    = 84
IMAGE_WIDTH     = 84
NUM_ENVS        = 2          # ← parallel environments (big GPU win)
...
class DrivingCNNMLP(BaseFeaturesExtractor):
    ...
    def forward(self, observations):
        img   = observations["image"]   # (B, H, W, C, stack)
        state = observations["state"]   # (B, lidar_dim)

        # Drop stack dim, convert to (B, C, H, W) — contiguous for CUDA
        img = img[..., 0].permute(0, 3, 1, 2).float().contiguous()
...
# ─────────────────────────────────────────────
#  ENV FACTORY  (must be top-level for SubprocVecEnv)
# ─────────────────────────────────────────────
def make_env(rank, render=False):
    """Returns a callable that creates a single monitored env."""
    def _init():
...
def train():
    ...
    vec_env = SubprocVecEnv(
        [make_env(rank=i, render=False) for i in range(NUM_ENVS)],
        start_method="fork"   # use "spawn" on Windows
    )
    ...
    model = PPO(
        ...
        learning_rate   = 3e-4,
        # NUM_ENVS × n_steps = total samples per update
        # 8 × 512 = 4096 → same as before but collected in parallel
        n_steps         = 2048,
        batch_size      = 256,   # larger minibatch → better GPU utilisation
        n_epochs        = 10,
        gamma           = 0.99,
        gae_lambda      = 0.95,
        clip_range      = 0.2,
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq   = SAVE_FREQ // NUM_ENVS,   # SB3 counts per-env steps
        save_path   = MODEL_DIR,
        name_prefix = MODEL_NAME,
        verbose     = 1,
    )

    device_label = f"GPU ✅  ({torch.cuda.get_device_name(0)})" if torch.cuda.is_available() else "CPU ⚠️"
    print("🚗 Starting Vision + Lidar Training...\n")
    print(f"   Device           : {device_label}")
    print(f"   Parallel envs    : {NUM_ENVS}")
    print(f"   Samples/update   : {NUM_ENVS * 512:,}  ({NUM_ENVS} envs × 512 steps)")
